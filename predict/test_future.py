...
def main(numbers: int):
    ...
    results_1h = procedure_calculate_euclidean(results_4h, "1h", type="future")
    if not results_1h[1]:
        return "No result"
    results_15m = procedure_calculate_euclidean(results_1h, "15m", type="future")
    if not results_15m[1]:
        return "No result"
    results_5m = procedure_calculate_euclidean(results_15m, "5m", type="future")
    if not results_5m[1]:
        return "No result"
    results_1m = procedure_calculate_euclidean(results_5m, "1m", type="future")
    if not results_1m[1]:
        return "No result"
