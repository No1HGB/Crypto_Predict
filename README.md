...
=> 6/20 $729 => 7/26 $950 (개입 안함. 오직 알고리즘.)
####
- ???: 그럼 병신아 금융기관에서 그걸 하겠지 => 기술적 분석은 금융기관에서 펀드매니저들이 과거에 계속 하던 방식이고 지금도 다 하고 있음. 실제로 미국장의 80%(2024년) 정도는 이미 알고리즘에 의한 거래.
...
####
- 최근엔 자산운용사 중심으로 딥러닝 이용해서 투자하고 있다고 함(경제학 교수 피셜)
=> 트랜스포머 변형 등 다양하게 하고 있다 카더라..
####
- 그럼 나도 해볼 수 있는거 아님?
--------
## 단기투자 기본 지식
...
####
- 단기적 관점에서 왜 상승 모멘텀, 하락 모멘텀이 존재하나? => 사람들은 군중심리에 의해 가격이 상승할 때 사려는 경향을 보이고, 내려갈 때 팔고 싶은 경향을 보임(FOMO: Fear Of Missing Out)
####
- 즉, 단기적 관점에서 가격은 랜덤이며 예측은 불가능하다는 '효율적 시장 가설 이론'은 맞지 않음(장기적으로는 맞음)
####
- 만약 가격이 정말 랜덤이라면 상승 모멘텀과 하락 모멘텀이 존재해서는 안됨. 그러나 현실에선 상승, 하락, 횡보가 번갈아가며 나타남.
--------
## 기본 가정
- 가정1: 투자자들의 '큰 심리'는 인간의 심리이므로 비슷하다
=> 크게 오르면 '사야 하나?', 크게 떨어지면 '팔아야 하나?'
####
...
- 50%보다 약간 높게만 예측해도 큰 수의 법칙에 의해 돈을 벌 수 있다.
- 목표: 60%: 성공 / 70%: 미친 성공 / 80% 이상: 흠.. 불가능하지 않을까..
####
???: 이게 가능했으면 이미 금융기관이 하고 있는데 개인이 돈을 벌 수가 있음?
- 당연히 하고 있겠지만, 금융기관 입장에서 하루 뒤 예측은 의미가 없음.
- 포지션이 매우 크기 때문에 하루 만에 포지션 청산이 불가능해(물량을 못 받음. 하루만에는) 조금 더 긴 기간을 예측해야 의미가 있음.
- 그리고 알겠지만 조금 더 긴 기간을 예측하려고 할수록 정확도는 떨어지게 됨.
- 반면 개인은 운용자금이 작기 때문에, 하루 단위 예측을 하게 되면 포지션 청산을 통해서 돈을 벌 수 있음.
--------
## 데이터 설명
...
- 사실 이미 주식 데이터 자체는 전처리를 통해서 적정 스케일이 되어있는 결과임
### 기타 다른 모델
- 회귀식 찾는거랑 똑같은 거 아님? => 순차모델 사용해서 회귀로 풀어봤으나 2차원 합성곱이 더 잘 맞음
- RNN(LSTM,GRU) => 계산량은 더럽게 많고 잘 맞지도 않음. 실제로 여타 논문들에서도 정확도가 상당히 떨어짐
- Transformer => 아직 안해봄. 정확히는 어떻게 Attention을 주어야 할 지 아이디어가 없음. 아이디어좀.
-------

## 기본 설정

- 회사니까 CUDA와 cuDNN 다 설치되어 있을거임(환경설정도 다 되어있겠지 당연히)
- requirements.txt에 필요 패키지 있음. 아마 필수적인 것들이라 추가 설치 필요 없을듯
- 나는 Apple Silicon 칩이라 tensorflow-gpu는 설치 못했음, 그니까 tensorflow gpu만 설치 해주셈
=> pip install tensorflow-gpu
- 변수 설명은 main.py에 있음. 하나의 모듈을 하나의 파일로 다 붙여놓은 거라 가독성은 이해좀;
